...
export const Member = ({ showing, member: { ref, id, emails, name, status, practices, team } }: Props) => {
  ...
  const onInputChange = (event: React.ChangeEvent<HTMLInputElement>, field: string) => {
    switch (field) {
      case 'name':
    ...
    <div className="w-full h-screen absolute bg-blueGray-700 bg-opacity-50 backdrop-blur-md text-blueGray-300 flex flex-col">
      ...
      <div className="flex-grow items-end flex px-6">
        <div className="w-full self-center bg-white rounded-xl p-4 flex flex-col space-y-2 border-b-[6px] border-cyan-600">
          <input className="apperance-none text-2xl w-full focus:outline-none px-2 font-bold text-blueGray-600 placeholder-current" placeholder="Mason Munoz" />
        </div>
      </div>
